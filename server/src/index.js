...
app.get('/app/mall/:mallSlug/parking/zones', async (req, res) => {
  const { rows } = await query('SELECT id,zone_code,level_label,capacity FROM parking_zones WHERE tenant_id=$1::uuid ORDER BY zone_code', [req.tenant.id]);
  res.json(rows);
});
app.post('/app/mall/:mallSlug/parking/save-car', requireAuth, async (req, res) => {
  await query('INSERT INTO car_locations(id,tenant_id,user_id,parking_zone_id,notes) VALUES($1::uuid,$2::uuid,$3::uuid,$4::uuid,$5)', [createId(), req.tenant.id, req.user.id, req.body.parkingZoneId, req.body.notes || '']);
  res.status(201).json({ ok: true });
});
app.get('/app/mall/:mallSlug/parking/find-car', requireAuth, async (req, res) => {
  const { rows } = await query(
    `SELECT cl.id,cl.notes,cl.created_at,pz.zone_code,pz.level_label
     FROM car_locations cl
     JOIN parking_zones pz ON pz.id=cl.parking_zone_id
     WHERE cl.tenant_id=$1::uuid AND cl.user_id=$2::uuid
     ORDER BY cl.created_at DESC LIMIT 1`,
    [req.tenant.id, req.user.id]
  );
  if (!rows[0]) return res.status(404).json({ error: 'No saved car location' });
  res.json(rows[0]);
});

app.get('/app/mall/:mallSlug/messages', requireAuth, async (req, res) => {
  const { rows } = await query(
    `SELECT t.id AS thread_id,
            (SELECT m.body FROM messages m WHERE m.thread_id=t.id ORDER BY m.created_at DESC LIMIT 1) AS last_message,
            (SELECT m.created_at FROM messages m WHERE m.thread_id=t.id ORDER BY m.created_at DESC LIMIT 1) AS last_message_at
...
app.post('/app/mall/:mallSlug/messages/:threadId/reply', requireAuth, async (req, res) => {
  await query('INSERT INTO messages(id,tenant_id,thread_id,sender_user_id,body) VALUES($1::uuid,$2::uuid,$3::uuid,$4::uuid,$5)', [createId(), req.tenant.id, req.params.threadId, req.user.id, req.body.body]);
  res.status(201).json({ ok: true });
});
